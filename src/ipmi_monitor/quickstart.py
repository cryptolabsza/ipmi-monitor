...
def check_root():
    ...
    if os.geteuid() != 0:
        ...
        console.print("Run with: [cyan]sudo ipmi-monitor setup[/cyan]")
        sys.exit(1)
...
def run_quickstart(config_path: str = None, yes_mode: bool = False):
    ...
    # was already deployed by dc-overview setup or a prior ipmi-monitor run
...
def _deploy_server_manager(
    ...
    Unlike dc-overview setup, this does NOT set up Prometheus, Grafana,
...
def generate_proxy_nginx_config(domain: str, use_letsencrypt: bool = False) -> str:
    ...
    prometheus) will be added by dc-overview setup when installed.
    """
...
# Generated by: ipmi-monitor setup
...
def prompt_ssh_key(custom_style) -> Optional[str]:
    ...
    if action_type == 'select':
        ...
        if not proceed:
            console.print("[dim]You can add the key later and re-run setup.[/dim]")
